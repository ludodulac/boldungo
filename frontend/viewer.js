...
function noticeStep12SupportEvidence(state){
  ...
  for(const id of state.added){
    ...
    const supports=[...state.before].map(pid=>partsById.get(pid)).filter(Boolean).filter(candidate=>candidate.z_plates+dims(candidate).heightPlates===part.z_plates&&overlap(area,footprint(candidate)));
